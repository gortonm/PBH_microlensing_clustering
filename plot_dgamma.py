...
n_cls = 1e6    # for Fig. 1, only show N_cl = 1e5

r_small_m_pbh_1, r_small_m_pbh_10 = 2, 2
# choose realisations to plot (these depend on the PBH mass)
realisations_ncl_1e6_mpbh_1 = np.array([0, 1, r_small_m_pbh_1])
realisations_ncl_1e6_mpbh_10 = np.array([0, 1, r_small_m_pbh_10])
scale=1
...
for k, m_pbh in enumerate(m_pbhs):
    ...
    for n_cl in n_cls:
        ...
        for i, r in enumerate(realisations):
            ...
            filename = f'{os.getcwd()}' + '/simulated_data_constraints/N_cl/{0:.2f}'.format(np.log10(n_cl)) + '/M_PBH/{0:.2f}/'.format(np.log10(m_pbh)) + 'cluster_data_update_pcl_' + str(r) + '.csv'
