...
def save(d_L, v, n_cl, m_pbh):
    ...
    np.savetxt(filepath + 'updateRE_dL.txt', d_L)
    np.savetxt(filepath + 'updateRE_v.txt', v)
...
for n_cl in n_cls:
    for m_pbh in m_pbhs:
        ...
        for f_pbh in np.array([1]):
            ...
            np.savetxt(filepath + 'n_ex_EROS_2_fpbh={0:.3f}_1e4samples_factor_updateRE.txt'.format(f_pbh), n_ex_EROS_efficiency)
            np.savetxt(filepath + 'n_ex_perfect_fpbh={0:.3f}_1e4samples_factor_updateRE.txt', n_ex_perfect_efficiency)
