...
def setup_fpbh(m_pbh):
    ...
    return np.arange(f_pbh_mid-0.05, f_pbh_mid+0.06, 0.01)


def save(d_L, v, n_cl, m_pbh, f_pbh):
    """
    ...
    np.savetxt(filepath + '_dL.txt')
    np.savetxt(filepath + '_v.txt')
    ...
    for f_pbh in setup_fpbh(m_pbh):
        for n_cl in n_cls:
            ...
            for i in range(0, n_realisations):
                ...
                d_L, v = produce_values(n_cl, m_pbh, d_s, v_c, f_pbh)
            ...
            np.savetxt(filepath + 'n_ex_EROS_2_fpbh={0:.2f}.txt'.format(f_pbh), n_ex_EROS_efficiency)
