...
for k, m_pbh in enumerate(m_pbhs):
    ...
    for i, r in enumerate(realisations):
        ...
        if r == 12:
            ax.set_ylim(0, scale * max(dgamma_smooth) * 2)
        if r == 16:
            ax.set_ylim(0, scale * max(dgamma_smooth) * 2.5)
    ...
    plt.tight_layout()
    plt.savefig(f'{os.getcwd()}' + '/figures/n_cl=1e{:.0f}_mpbh=1e{:.0f}'.format(np.log10(n_cl), np.log10(m_pbh)) + '_15to19.pdf')    
...
bin_spacing = 10000 / (365.25 * np.sqrt(len(t_hat_values_corrected)))
                        
ax.plot(np.array(t_smooth)/365.25, scale * np.array(dgamma_smooth), color='k', linewidth=0.7)
ax.hist(np.array(t_hat_values_corrected), weights = scale * np.array(event_rate_values_corrected) / (bin_spacing), color='tab:blue', histtype='step', bins = np.arange(0, (10000/365.25)+bin_spacing/2, bin_spacing))
ax.hist(np.array(t_hat_values_uncorrected), weights = scale * np.array(event_rate_values_uncorrected) / (bin_spacing), color='tab:orange', histtype='step', linewidth = 0.5, bins = np.arange(0, (10000/365.25)+bin_spacing/2, bin_spacing))        
ax.set_ylim(0, scale * max(dgamma_smooth) * 2)
plt.xlim(0, 10000/365.25)